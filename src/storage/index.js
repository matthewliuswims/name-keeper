...
const validateAsString = (input) => {
  ...
  if (!validString) throw Error(`expected input to be valid string, got ${input}`)
  return true
}

const validateAsObject = (input) => {
  const validItem = input && (typeof input == 'object') && input.hasOwnProperty('id')
  if (!validItem) throw Error(`expected input to be valid object, got ${input}`)
  return true
}
 ...
 * @param {string} resource 
 */
export const editItem = async ({ key, item: itemParam, resource: resourceParam }) => {
  validateAsString(key) && validateAsString(resourceParam) && validateAsObject(itemParam)
